...
# Function to calculate the angle between the wrist and the forward/backward direction
def calculate_wrist_forward_backward_angle(wrist, hand_center):
    # Vector from wrist to hand center
    forward_vector = (hand_center[0] - wrist[0], hand_center[1] - wrist[1], hand_center[2] - wrist[2])
    
    # Vector in the forward/backward direction (along the z-axis)
    forward_direction = (0, 0, 1)
    
    # Dot product of the two vectors
    dot_product = forward_vector[2]
    
    # Magnitude of the vectors
    magnitude_forward_vector = math.sqrt(forward_vector[0]**2 + forward_vector[1]**2 + forward_vector[2]**2)
    magnitude_forward_direction = 1
    
    # Angle between the vectors
    angle = math.degrees(math.acos(dot_product / (magnitude_forward_vector * magnitude_forward_direction)))
    
    return angle

# Function to calculate the angle between the wrist and the left/right direction
def calculate_wrist_left_right_angle(wrist, hand_center):
    # Vector from wrist to hand center
    left_right_vector = (hand_center[0] - wrist[0], hand_center[1] - wrist[1], 0)
    
    # Vector in the left/right direction (along the x-axis)
    left_right_direction = (1, 0, 0)
    
    # Dot product of the two vectors
    dot_product = left_right_vector[0]
    
    # Magnitude of the vectors
    magnitude_left_right_vector = math.sqrt(left_right_vector[0]**2 + left_right_vector[1]**2)
    magnitude_left_right_direction = 1
    
    # Angle between the vectors
    angle = math.degrees(math.acos(dot_product / (magnitude_left_right_vector * magnitude_left_right_direction)))
...
def lines_and_box_3d(image, points):
    ...
    wrist = points[0]
    hand_center = points[9]  # Assuming the 9th landmark is the hand center
    forward_backward_angle = calculate_wrist_forward_backward_angle(wrist, hand_center)
    left_right_angle = calculate_wrist_left_right_angle(wrist, hand_center)
    angles.append(forward_backward_angle)
    angles.append(left_right_angle)
...
class HandTrackerApp:
    ...
    def save_to_csv(self):
        if self.csv_data:
            # Column names updated to include bottom joint angles per finger and wrist angles
            ...
            wrist_angles = ['Wrist_Forward_Backward_Angle', 'Wrist_Left_Right_Angle']
            column_names.extend(bottom_joint_angles)
            column_names.extend(wrist_angles)
